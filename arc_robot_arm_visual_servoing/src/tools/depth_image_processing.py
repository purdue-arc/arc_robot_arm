...
class DepthImageProcessing:
	...
	def __init__(self):
		...
		self.depth_image_sub = rospy.Subscriber(depth_sub_topic, Image, self.image_depth_cb)
		self.depth_confidence_sub = rospy.Subscriber(confidence_sub_topic, Image, self.depth_confidence_cb)
		self.depth_info_sub = rospy.Subscriber(depth_info_sub_topic, CameraInfo, self.image_depth_info_cb)
		
		self.image_sub = rospy.Subscriber(image_sub_topic, Image, self.image_cb)
		self.detections_sub = rospy.Subscriber(detections_topic, BoundingBoxes, self.detections_cb)
	...
	def image_depth_cb(self, data):
		...
		try:
			...
			if self.intrinsics and self.object_center is not None:
				...
				line += 'Depth at pixel(%3d, %3d): %7.1f(mm).' % (target_pix[0], target_pix[1], self.depth)
	...
	def image_cb(self, image_data):
		...
		try:
			...
			if self.bbox is not None:	
				x_min,x_max,y_min,y_max = self.bbox.xmin, self.bbox.xmax, self.bbox.ymin, self.bbox.ymax
				self.object_center = (round((x_min + x_max) / 2.0), round((y_min + y_max) / 2.0))
				self.frame = cv2.drawMarker(self.frame, self.object_center, (0,255,0), 
							markerSize=20, thickness=2, line_type=cv2.MARKER_CROSS)

			height, width, dims = self.frame.shape
			self.camera_center = (round(width/2),round(height/2))

			self.frame = cv2.drawMarker(self.frame, self.camera_center, (0,0,255), 
