...
import rospkg
import rospy
import relaxed_ik_ros1.utils as utils
import tf
...
grasp_val = 0.0 # 0.0 is closed -1 is open
def grasp_update_cb(msg):
    global grasp_val
    grasp_val = msg.data
...
def main():
    rospy.init_node('ik_publisher_node')
    ...
    is_sim = rospy.get_param("/sim",0)
    joint_state_topic = 'joint_states' if is_sim == 1 else 'desired_joint_states'

    js_pub = rospy.Publisher(joint_state_topic, JointState,queue_size=5)
    ...
    rospy.Subscriber('/grasp', Float64, grasp_update_cb)
